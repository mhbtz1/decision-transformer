...
def attention(query, key, value, mask=None, dropout: Optional[nn.Module] = None):
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, memory, src_mask, tgt_mask):
        ...
        x = self.sublayer[1](x, lambda x: self.src_attn(x, m, m, src_mask)) # take in the 
...
class EncoderDecoder(nn.Module):
    ...
    def decode(self, mem, src_mask, target, target_mask):
        return self.decode(self.target_embed(target), mem, src_mask, target_mask)
    def forward(self, src, target, src_mask, target_mask):
        return self.decoder(self.encoder(src, src_mask), src_mask, target, target_mask)
